...
// 将数据写入文件   
try {   
    FileWriter fw = new FileWriter(str1,true);  //第二个参数true代表追加写入,false或者不写代表覆盖写入 
    fw.write(str2);   
    fw.flush();   
    fw.close();    
} catch (IOException e) {   
...
```
## 12.枚举一个文件夹中子文件目录和子文件
```java
//12.枚举一个文件夹中的子文件目录和子文件  
//import java.io.*; 
File file = new File("C:\\Users\\Administrator\\Desktop"); //通过文件路径来创建一个File类
    if (file.isDirectory()) { //判断是否是目录
        File[] fileArr = file.listFiles(); //获取该文件目录下的子文件目录和子文件，若file不是目录则会报错
