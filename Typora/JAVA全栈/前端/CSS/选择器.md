- 通配选择器：*，代表所有；\*{color:red}，所有元素的字体颜色为红色

- id选择器：#id，选择对应id的元素；#id1{color:red}，标签id="id1"的元素字体为红色
- 类选择器：.class，选择使用了相应class的元素；.class1{color:red}，标签class="class1"的元素字体为红色
- 标签选择器：element，选择<element>标签元素；p{color:red}，所有<p></p>>标签内字体为红色
- 属性选择器：[attribute]，选择带有attribute属性的元素；[name]{color:red}，所有带有name属性的元素的字体都为红色

- 组合选择器：（不限于标签选择器，而且组合选择器之间也可以组合）
  - 后代选择器：div p{color:red}，div标签下的所有p标签内的字体为红色（不分层级）
  - 子选择器：div>p{color:red}，div标签内第一层的p标签的字体为红色（分层级）
  - 多元素选择器：div,p{color:red}，div和p标签的字体都为红色
  - 毗邻选择器：div+p{color:red}，跟div同级、相邻的下一个p标签字体为红色（紧挨着，找相邻的,只找下面的，不找上面的）
  - 兄弟选择器：div~p{color:red}，跟div同级的下面的所有p标签字体为红色（不需紧挨着，只找下面的，不找上面的）
